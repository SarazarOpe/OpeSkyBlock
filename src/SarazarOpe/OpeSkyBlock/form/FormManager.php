...
namespace SarazarOpe\OpeSkyBlock\form;

use SarazarOpe\OpeSkyBlock\OpeSkyBlock;
use SarazarOpe\OpeSkyBlock\utils\Utils;
use SarazarOpe\FormBuilderX\SimpleForm;
use SarazarOpe\FormBuilderX\CustomForm;
...
class FormManager {
  ...
  public function startForm(Player $player) {
    ...
    $form = new SimpleForm(function (Player $player, $data) use ($skyblock) {
      ...
      switch ($data) {
        case 0:
          if (!$player->hasPermission("osb.create")) return;
          ...
          break;
        case 1:
          if (!$player->hasPermission("osb.teleport")) return;
          $this->openTeleport($player);
          break;
        case 2:
          if (!$player->hasPermission("osb.settings")) return;
          if ($skyblock->haveIsland($player->getName())) {
            $this->openSettings($player);
          } else $player->sendMessage(Utils::getMessage("island-not-exists"));
          break;
        case 3:
          if (!$player->hasPermission("osb.top")) return;
          $this->openTop($player);
          break;
        case 4:
          if (!$player->hasPermission("osb.info")) return;
          $this->openInfo($player);
          break;
        case 5:
          if (!$player->hasPermission("osb.delete")) return;
    ...
    if ($player->hasPermission("osb.delete")) {
  ...
  public function openCreate(Player $player) {
    ...
    $form = new CustomForm(function (Player $player, $data) use ($skyblock) {
      if ($data === null) return;
      $path = OpeSkyBlock::getInstance()->getDataFolder() . "/world";
      $type = scandir($path);
    ...
    $form = new CustomForm(function (Player $player, $data) use ($skyblock) {
      ...
      if (isset($data[1]) && is_string($data[1])) {
        if ($skyblock->haveIsland($data[1])) {
          if (!$skyblock->isLocked($data[1]) || $player->hasPermission("osb.bybass")) {
            if (!$skyblock->isBanned($data[1], $player->getName()) || $player->hasPermission("osb.bybass")) {
  ...
  public function openSettings(Player $player) {
    ...
    $form = new SimpleForm(function (Player $player, $data) use ($skyblock) {
      ...
      switch ($data) {
        case 0:
          if (!$player->hasPermission("osb.settings.setspawn")) return;
          ...
          break;
        case 1:
          if (!$player->hasPermission("osb.settings.friends")) return;
          $this->openFriendsManager($player);
          break;
        case 2:
          if (!$player->hasPermission("osb.settings.kick")) return;
          $this->openKick($player);
          break;
        case 3:
          if (!$player->hasPermission("osb.settings.lock")) return;
          $skyblock->isLocked($player->getName()) ? $skyblock->unLocked($player->getName()) : $skyblock->setLocked($player->getName());
          break;
        case 4:
          if (!$player->hasPermission("osb.settings.pvp")) return;
          $skyblock->allowPvP($player->getName()) ? $skyblock->lockPvP($player->getName()) : $skyblock->unlockPvP($player->getName());
          break;
        case 5:
          if (!$player->hasPermission("osb.settings.ban")) return;
          $this->openBan($player);
          break;
        case 6:
          if (!$player->hasPermission("osb.settings.setwelcome")) return;
